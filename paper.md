...
Λ
CDM
ΛCDM or fragmented across specialized codes that are difficult to couple with modern Bayesian samplers.

Aeternvm Vacuvm provides researchers and independent scientists with a self-contained, modular Python engine to:

Solve non-linear scalar field evolution and background dynamics deterministically.
Evaluate local gravitational suppression via Vainshtein screening mechanics.
Accelerate cosmological parameter inference using optimized Gaussian Process emulators integrated with frameworks like Cobaya and EFTCAMB.
...
The depletion field 
χ
χ is governed by a potential of the form:
...
Crucially, the energy scale 
V
0
V 
0
​
  is not treated as a free tuning parameter. Instead, it is analytically constrained by the vacuum impedance 
Z
0
=
μ
...
inst
​
 ≈280 represents a non-perturbative instanton-like action. This yields 
V
0
∼
10
...
−47
 GeV 
4
 , naturally matching the observed dark energy density scale without fine-tuning.
...
The Klein-Gordon equation describing the evolution of 
χ
χ is solved numerically via high-precision adaptive ODE/PDE routines (such as LSODA and DOP853):
...
a
​
 CDM, this package is designed for screening mechanisms where the effective gravitational coupling 
μ
e
f
f
...
eff
​
 (k,a) is scale and time dependent.
